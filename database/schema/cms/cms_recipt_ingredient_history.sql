...
CREATE TABLE sous_chef_cms.cms_recipe_ingredient_history(
    id SERIAL NOT NULL,
    history_recipe_id INT8 NOT NULL,
...
COMMENT ON COLUMN sous_chef_cms.cms_recipe_ingredient_history.history_recipe_id IS 'History recipe id;History dish ID (FK, history_dish_id -> cooking_history_dishes.id)';
COMMENT ON COLUMN sous_chef_cms.cms_recipe_ingredient_history.ingredient_id IS 'Ingredient id;Ingredient ID (FK, ingredient_id -> ingredients.id)';
...
CREATE INDEX gk_recipe_ingredient_history_dish_id ON sous_chef_cms.cms_recipe_ingredient_history (
    history_recipe_id ASC
);
COMMENT ON INDEX sous_chef_cms.gk_recipe_ingredient_history_dish_id IS 'General index of history dish id;The general index of history_recipe_id FK to id in recipe table';
