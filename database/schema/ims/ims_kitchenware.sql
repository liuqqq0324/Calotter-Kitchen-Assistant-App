...
COMMENT ON COLUMN sous_chef_ims.ims_kitchenware.user_id IS 'User id;Refer to the account owner user id (FK, user_id -> users.id)';
COMMENT ON COLUMN sous_chef_ims.ims_kitchenware.kitchenware_id IS 'Kitchenware id;Associate to global kitchenware table (FK, kitchenware_id -> kitchenware.id)';
...
COMMENT ON TABLE sous_chef_ims.ims_kitchenware IS 'ims_kitchenware;This table stores all user kitchenware, with each record containing the basic information of the kitchenware and the user to whom it belongs.';
