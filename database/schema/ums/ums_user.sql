...
COMMENT ON COLUMN sous_chef_ums.ums_user.email IS 'User email;Registration email (for login, receiving notification, and retrieving password)';
...
CREATE  UNIQUE INDEX uk_users_username ON sous_chef_ums.ums_user (
    username ASC
);
COMMENT ON INDEX sous_chef_ums.uk_users_username IS 'Unique index of username;The unique index of username field in user table';
CREATE  UNIQUE INDEX uk_users_email ON sous_chef_ums.ums_user (
    email ASC
);
COMMENT ON INDEX sous_chef_ums.uk_users_email IS 'Unique index of email;The unique index of email field in user table';