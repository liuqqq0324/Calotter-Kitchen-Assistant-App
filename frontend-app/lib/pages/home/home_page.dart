...
import 'package:personal_sous_chef/widgets/programmatic_sketchy_card.dart';
  ...
  Widget _buildTrackingButton({
    ...
    return GestureDetector(
      ...
      child: Container(
        ...
        decoration: BoxDecoration(
          color: const Color(0xFFFFFFF0), // Paper White（略带暖色）
          borderRadius: BorderRadius.circular(6),
          border: Border.all(
            color: const Color(0xFF6B4F4F).withOpacity(0.25), // River Deep Brown
            width: 1,
          ),
          boxShadow: [
            BoxShadow(
        ...
        child: Column(
          ...
          children: [
                // 2) 图案区域：先用 IconData（你后续换 PNG/SVG 只需替换这一块）
                Expanded(
                  child: Center(
                    child: Container(
                      width: 64,
                      ...
                      child: Icon(
                        icon,
                        size: 40,
                        color: const Color(0xFF6B4F4F).withOpacity(0.85), // 统一“墨水”色
                      ),
                    ),
                  ),
                ),
...
class _GridPaperPainter extends CustomPainter {
  ...
  Path _createIrregularPath(Size size, math.Random random) {
    ...
    for (double x = step; x < effectiveWidth; x += step) {
      ...
      final y = noise.clamp(-edgeNoise, edgeNoise).toDouble(); // 限制偏移范围并转换为 double
      path.lineTo(x, y);
    }
