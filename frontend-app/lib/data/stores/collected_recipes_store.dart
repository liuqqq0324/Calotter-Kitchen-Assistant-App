...
import 'package:personal_sous_chef/services/api/favorite_recipes_api_service.dart';
...
class CollectedRecipesStore {
  ...
  static Future<RecipeModel> add(
    ...
    final saved = await FavoriteRecipesApiService.addFavorite(
      recipe,
      householdId: householdId,
    );
  ...
  static Future<RecipeModel?> toggle(
    RecipeModel recipe, {
    required int householdId,
  }) async {
    if (isCollected(recipe)) {
