...
import 'dart:convert';
// ⚠️ 已拆分：纯API调用已移至 services/api/household_api_service.dart
...
import 'package:personal_sous_chef/services/api/household_api_service.dart';
...
class HouseholdService {
  ...
  static Future<int?> getHouseholdId({String? userId}) async {
    try {
      ...
      // ⚠️ 已拆分：纯API调用已移至 services/api/household_api_service.dart
      // 请使用 HouseholdApiService.getCurrentHousehold() 进行HTTP请求
      ...
      // 使用新的API服务层
      final response = await HouseholdApiService.getCurrentHousehold(
        userId: userIdParam,
        token: token,
      );
  ...
  static Future<Map<String, dynamic>> inviteUser({
    ...
    try {
      ...
      // ⚠️ 已拆分：纯API调用已移至 services/api/household_api_service.dart
      // 请使用 HouseholdApiService.inviteUser() 进行HTTP请求
      // final url = Uri.parse(
      ...
      // 使用新的API服务层
      final response = await HouseholdApiService.inviteUser(
  ...
  static Future<Map<String, dynamic>> joinHousehold({
    ...
    try {
      ...
      // ⚠️ 已拆分：纯API调用已移至 services/api/household_api_service.dart
      // 请使用 HouseholdApiService.joinHousehold() 进行HTTP请求
      // final url = Uri.parse(
      ...
      // 使用新的API服务层
      final response = await HouseholdApiService.joinHousehold(
      ...
      if (response.statusCode == 200 && data['code'] == 200) {
        ...
        return {'success': false, 'error': data['message'] ?? 'Failed to join'};
      }
    } catch (e) {
      return {'success': false, 'error': 'Network error: $e'};
    }
  ...
  static Future<Map<String, dynamic>> leaveHousehold({
    ...
    try {
      ...
      // ⚠️ 已拆分：纯API调用已移至 services/api/household_api_service.dart
      // 请使用 HouseholdApiService.leaveHousehold() 进行HTTP请求
      // final url = Uri.parse(
      ...
      // 使用新的API服务层
      final response = await HouseholdApiService.leaveHousehold(
        householdId: householdId,
        userId: userId,
        token: token,
      );
  ...
  static Future<Map<String, dynamic>> removeMember({
    ...
    try {
      ...
      // ⚠️ 已拆分：纯API调用已移至 services/api/household_api_service.dart
      // 请使用 HouseholdApiService.removeMember() 进行HTTP请求
      // final url = Uri.parse(
      ...
      // 使用新的API服务层
      final response = await HouseholdApiService.removeMember(
        householdId: householdId,
        memberId: memberId,
        ownerId: userId,
        token: token,
      ...
      final data = jsonDecode(response.body);
      if (response.statusCode == 200 && data['code'] == 200) {
        return {'success': true, 'message': data['message'] ?? '移除成功'};
      } else {
        return {'success': false, 'error': data['message'] ?? '移除失败'};
      }
    } catch (e) {
      return {'success': false, 'error': 'Network error: $e'};
    }
  ...
  static Future<Map<String, dynamic>> switchCurrentHousehold({
    ...
    try {
      ...
      // ⚠️ 已拆分：纯API调用已移至 services/api/household_api_service.dart
      // 请使用 HouseholdApiService.switchCurrentHousehold() 进行HTTP请求
      // final url = Uri.parse(
      ...
      // 使用新的API服务层
      final response = await HouseholdApiService.switchCurrentHousehold(
        householdId: householdId,
        userId: userId,
        token: token,
      );
      ...
      if (response.statusCode == 200 && data['code'] == 200) {
        ...
        return {'success': false, 'error': data['message'] ?? '切换失败'};
      }
    } catch (e) {
      return {'success': false, 'error': 'Network error: $e'};
    }
  ...
  static Future<Map<String, dynamic>> getCurrentHousehold() async {
    try {
      ...
      if (response.statusCode == 200 && data['code'] == 200) {
        ...
        return {'success': false, 'error': data['message'] ?? '获取当前厨房失败'};
      }
    } catch (e) {
      return {'success': false, 'error': 'Network error: $e'};
    }
  ...
  static Future<Map<String, dynamic>> getJoinedHouseholds() async {
    try {
      ...
      // ⚠️ 已拆分：纯API调用已移至 services/api/household_api_service.dart
      // 请使用 HouseholdApiService.getJoinedHouseholds() 进行HTTP请求
      // final url = Uri.parse(
      ...
      // 使用新的API服务层
      final response = await HouseholdApiService.getJoinedHouseholds(
        userId: userId,
        token: token,
      );
  ...
  static Future<Map<String, dynamic>?> getHouseholdByInviteCode(
    ...
    try {
      // ⚠️ 已拆分：纯API调用已移至 services/api/household_api_service.dart
      // 请使用 HouseholdApiService.getHouseholdByInviteCode() 进行HTTP请求
      // final url = Uri.parse(
      ...
      // 使用新的API服务层
      final response = await HouseholdApiService.getHouseholdByInviteCode(
        inviteCode: inviteCode,
        token: token,
      );
  ...
  static Future<Map<String, dynamic>> regenerateInviteCode({
    ...
    try {
      ...
      // ⚠️ 已拆分：纯API调用已移至 services/api/household_api_service.dart
      // 请使用 HouseholdApiService.regenerateInviteCode() 进行HTTP请求
      // final url = Uri.parse(
      ...
      // 使用新的API服务层
      final response = await HouseholdApiService.regenerateInviteCode(
        householdId: householdId,
        ownerId: userId,
        token: token,
      );
      ...
      if (response.statusCode == 200 && data['code'] == 200) {
        ...
        return {'success': false, 'error': data['message'] ?? '重新生成失败'};
      }
    } catch (e) {
      return {'success': false, 'error': 'Network error: $e'};
    }
