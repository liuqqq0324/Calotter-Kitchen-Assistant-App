...
class CookingVoiceAssistant {
  ...
  Function(String)? _savedOnStatus; // 转发给 UI，用于无缝重启

  // Command keywords mapping (Simplified single-word English)
  static const Map<VoiceCommandType, List<String>> _commandKeywords = {
    VoiceCommandType.nextStep: ['next', 'continue', 'forward'],
    VoiceCommandType.previousStep: ['back', 'previous'],
    VoiceCommandType.repeatStep: ['repeat', 'again'],
    VoiceCommandType.startTotalTimer: ['start', 'begin'],
    VoiceCommandType.pauseTotalTimer: ['pause', 'wait'],
    VoiceCommandType.resumeTotalTimer: ['resume', 'continue'],
    VoiceCommandType.stopTotalTimer: ['stop', 'reset'],
    VoiceCommandType.startTimer: ['timer start', 'timer begin'],
    VoiceCommandType.pauseTimer: ['timer pause', 'timer wait'],
    VoiceCommandType.resumeTimer: ['timer resume'],
    VoiceCommandType.stopTimer: ['timer stop', 'timer reset'],
    VoiceCommandType.completeStep: ['done', 'finish', 'complete'],
    VoiceCommandType.nextDish: ['dish', 'recipe', 'switch'],
    VoiceCommandType.previousDish: ['last', 'before'],
    VoiceCommandType.currentStepInfo: ['step', 'info'],
    VoiceCommandType.timerStatus: ['time', 'left'],
    VoiceCommandType.ingredientsList: ['ingredients', 'needs'],
    VoiceCommandType.exitVoiceMode: ['exit', 'quit', 'close', 'off'],
    VoiceCommandType.help: ['help', 'commands'],
  };
  ...
  Future<void> startListening({
    ...
    Function(double level)? onSoundLevelUpdate,
    Function(String error)? onError,
    Function(String status)? onStatus, // 暴露给 UI，用于无缝重启
  }) async {
    ...
    try {
      await _speech.listen(
        ...
        pauseFor: const Duration(seconds: 5), // 说话停顿 5 秒再结束，避免 Android 过早关麦
        localeId: "en_US",
        cancelOnError: true,
        partialResults: true,
        onDevice: true, // 本机识别更稳定，无网可用（不支持时 listen 可能失败，由 onError 重连）
      );
  ...
  VoiceCommandType recognizeCommand(String text) {
    ...
    final stepNumberMatch = RegExp(
      r'(?:step|go to step|jump to step)\s*(\d+)',
      caseSensitive: false,
    ...
    if (normalizedText.contains('timer start') ||
        normalizedText.contains('timer begin')) {
      return VoiceCommandType.startTimer;
    }
    if (normalizedText.contains('timer stop') ||
        normalizedText.contains('timer reset')) {
      return VoiceCommandType.stopTimer;
    }
    if (normalizedText.contains('timer pause') ||
        normalizedText.contains('timer wait')) {
      return VoiceCommandType.pauseTimer;
    }
    if (normalizedText.contains('timer resume') ||
        normalizedText.contains('timer continue')) {
    ...
    for (final entry in _commandKeywords.entries) {
      if (entry.key == VoiceCommandType.nextDish ||
          entry.key == VoiceCommandType.previousDish) {
