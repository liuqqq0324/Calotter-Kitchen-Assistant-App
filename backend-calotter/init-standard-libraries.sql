-- ============================================
-- 标准库数据初始化脚本（完整版，仅包含 YOLO 模型标签）
-- Standard Libraries Data Initialization Script (Complete, YOLO model labels only)
-- ============================================
-- 
-- 使用方法:
...
VALUES 
  ...
  -- 单位规范：primary_unit='pcs', secondary_unit='g', unit_conversion_factor=average_gram_per_unit, standard_unit='g'
  ...
  -- Meat & Protein (MEAT) - ID: 1064-1083
  -- 单位规范：
  --   - 按重量计（g/kg）：primary_unit='g', secondary_unit='kg', unit_conversion_factor=0.001 (1 g = 0.001 kg), standard_unit='g'
  --   - 按个计（pcs/g）：primary_unit='pcs', secondary_unit='g', unit_conversion_factor=average_gram_per_unit, standard_unit='g'
  ...
  -- Grains & Starches (GRAIN) - ID: 1085-1091
  -- 单位规范：primary_unit='g', secondary_unit='kg', unit_conversion_factor=0.001 (1 g = 0.001 kg), standard_unit='g'
  ...
  -- Dairy & Others (DAIRY/OTHER) - ID: 1093-1096
  -- 单位规范：
  --   - 液体（ml/L）：primary_unit='ml', secondary_unit='L', unit_conversion_factor=0.001 (1 ml = 0.001 L), standard_unit='ml'
  --   - 固体（g/kg）：primary_unit='g', secondary_unit='kg', unit_conversion_factor=0.001 (1 g = 0.001 kg), standard_unit='g'
  --   - 按个计（pcs/g）：primary_unit='pcs', secondary_unit='g', unit_conversion_factor=average_gram_per_unit, standard_unit='g'
...
WHERE NOT EXISTS (
  ...
  WHERE ingredient_id = 1072 AND allergen_id = 3
);
...
WHERE NOT EXISTS (
  SELECT 1 FROM ingredient_allergens 
  WHERE ingredient_id = 1094 AND allergen_id = 2
);
